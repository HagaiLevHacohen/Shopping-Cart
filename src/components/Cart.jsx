      ...
      { isCartEmpty ?
        ...
        <div className={styles.orderContainer}>
          <div className={styles.orderProducts}>
              {itemsList.map((item) => item.id in cartItems ? <OrderCard key={item.id}
                image={item.image}
                title={item.title} 
                price={item.price}
                amount={cartItems[item.id]}
                SetAmount={(amount) => setCartItems({...cartItems, [item.id]:amount})}
                handleRemove={() => {
          ...
          <div className={styles.orderSummary}>
              ...
              <button className={styles.continueShoppingBtn} onClick={() => navigate('/Shop')}>Continue Shopping</button>
          </div>
        </div>
